...
def interp_group(group, col, method):
...
for val_date in val_dates:
    ...
    fold = fold.groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: interp_group(g, 'IV_simulated', method))
...
df_long.loc[mask_far, 'predicted_IV'] = df_long[mask_far].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: interp_group(g, 'IV', 'log'))['predicted_IV']
mask_near = df_long['date'] == near_expiry_date
df_long.loc[mask_near, 'predicted_IV'] = df_long[mask_near].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: interp_group(g, 'IV', 'linear'))['predicted_IV']
mask_expiry = df_long['date'] == expiry_date
df_long.loc[mask_expiry, 'predicted_IV'] = df_long[mask_expiry].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: interp_group(g, 'IV', 'akima'))['predicted_IV']
...
submission[['id', 'predicted_IV']].rename(columns={'predicted_IV': 'value'}).to_csv('submission_12.csv', index=False)
print('Pipeline complete. Saved as submission_12.csv')