...
df_long['IV_pcp'] = df_long.groupby(['datetime_obj', 'strike'])['IV'].transform(lambda x: x.fillna(x.mean(skipna=True)))
...
def apply_normal_spline(group, col):
    ...
    preds = (
        CubicSpline(x_obs, y_obs, bc_type='natural', extrapolate=True)(missing['strike'].values)
        if len(observed) >= 3
        else interp1d(x_obs, y_obs, kind='linear', fill_value='extrapolate')(missing['strike'].values)
    )
...
for val_date in val_dates:
    ...
    fold['IV_sim_pcp'] = fold.groupby(['datetime_obj', 'strike'])['IV_simulated'].transform(lambda x: x.fillna(x.mean(skipna=True)))
    fold['predicted_IV'] = fold['IV_sim_pcp']
    fn = apply_normal_spline if val_date < expiry_date else apply_expiry_linear
    fold = fold.groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: fn(g, 'IV_sim_pcp'))
    preds = fold.loc[mask, 'predicted_IV']
    truths = fold.loc[mask, 'IV']
    valid = ~preds.isnull()
    mse = mean_squared_error(truths[valid], preds[valid])
    cv_scores.append(mse)

df_long['predicted_IV'] = df_long['IV_pcp']
mask_normal = df_long['date'] < expiry_date
df_long.loc[mask_normal, 'predicted_IV'] = df_long[mask_normal].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: apply_normal_spline(g, 'IV_pcp'))['predicted_IV']
mask_expiry = df_long['date'] == expiry_date
df_long.loc[mask_expiry, 'predicted_IV'] = df_long[mask_expiry].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: apply_expiry_linear(g, 'IV_pcp'))['predicted_IV']
...
submission[['id', 'predicted_IV']].rename(columns={'predicted_IV': 'value'}).to_csv('submission_7.csv', index=False)
print('Pipeline complete. Saved as submission_7.csv')