...
def apply_normal_quant(group, col):
    ...
    ivs = observed[col].values
    group.loc[missing.index, 'predicted_IV'] = [
        ivs[0] if s < strikes[0]
        else ivs[-1] if s > strikes[-1]
        else pchip_interpolate(strikes, ivs, s)
        for s in missing['strike'].values
    ]
    return group

def apply_expiry_quant(group, col):
    observed = group.dropna(subset=[col]).sort_values('strike')
    missing = group[group[col].isnull()]
    if len(observed) < 2 or missing.empty:
        return group
    interp = interp1d(observed['strike'].values, observed[col].values, kind='linear', fill_value='extrapolate')
...
for val_date in val_dates:
    ...
    fn = apply_normal_quant if val_date < expiry_date else apply_expiry_quant
...
df_long.loc[mask_normal, 'predicted_IV'] = df_long[mask_normal].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: apply_normal_quant(g, 'IV'))['predicted_IV']
mask_expiry = df_long['date'] == expiry_date
df_long.loc[mask_expiry, 'predicted_IV'] = df_long[mask_expiry].groupby(['datetime_obj', 'option_type'], group_keys=False).apply(lambda g: apply_expiry_quant(g, 'IV'))['predicted_IV']
...
submission[['id', 'predicted_IV']].rename(columns={'predicted_IV': 'value'}).to_csv('submission_5.csv', index=False)
print('Pipeline complete. Saved as submission_5.csv')
